create table user (
    ...
    member_id int(10) not null comment 'Forum member identifier',
    ...
    is_administrator tinyint(1) default 0 comment 'If the user is an administrator',
    avatar_url varchar(256) default null comment 'URL pointing to the member avatar',
    last_token_check datetime default null comment 'The last date and time when the authentication token was validated',
    last_detail_update datetime default null comment 'The last date and time when the member details were updated',
    primary key (user_id),
